package whatisnewin.com.sun.source.util;

import com.sun.source.util.DocTreeScanner;
import com.sun.source.doctree.SystemPropertyTree;

/**
 * This source file was generated by WhatIsNewInJava.<br>
 *
 * This class provides an example call to each method in class {@link DocTreeScanner}
 * that were newly introduced in Java version 12.<br>
 *
 * {@link DocTreeScanner} is an old class but has new fields, constructors or methods.
 * @since 1.8
 * @see DocTreeScanner
 */
public final class WhatIsNewInDocTreeScanner<R, P>
{
  /**
   * Example call to new method {@link DocTreeScanner#visitSystemProperty(SystemPropertyTree, Object)}.
   * @since 12
   * @see DocTreeScanner#visitSystemProperty(SystemPropertyTree, Object)
   */
  public R visitSystemProperty(SystemPropertyTree node, P p)
  {
    DocTreeScanner<R, P> testee = $$$();

    R result = testee.visitSystemProperty(node, p);
    return result;
  }

  private DocTreeScanner<R, P> $$$()
  {
    return null;
  }
}
